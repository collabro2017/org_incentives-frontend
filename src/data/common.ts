import { fileTypes } from "../files/files-reducer";
...
export const countryEntry = (countryCode: string): Country | undefined => countryOptions.filter((option) => option.key === countryCode)[0];
...
export const fileTypeOptions: FileTypeOption[] = fileTypes.map((type) => ({
    key: type.toString(),
    value: type.toString(),
    text: type.toString()
}));
